...
export const TaskBoard: React.FC<TaskBoardProps> = ({ currentUser }) => {
  ...
  const confirmArchive = async () => {
    ...
    setIsArchiving(true);
    try {
        await API.updateTaskStatus(archiveConfirmId, 'archived');
    ...
    <div className="p-8 h-full flex flex-col bg-slate-50">
      ...
      {archiveConfirmId && (
        <DeleteConfirmModal 
          ...
          message="Ви впевнені? Завдання перестане відображатися на дошці, але залишиться в архіві." 
          onClose={() => setArchiveConfirmId(null)} 
          onConfirm={confirmArchive} 
          isDeleting={isArchiving}
          confirmText="В архів"
          confirmButtonClass="bg-slate-900 hover:bg-slate-800 shadow-slate-200"
        />
      )}
    </div>
  );
